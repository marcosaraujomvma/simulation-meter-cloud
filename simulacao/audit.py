#auditor
#MODULO RESPOSAVEL POR RECUPERAR DADOS DOS BANCOS DE DADOS
# -*- coding: utf-8 -*-
...
lista_id_medidor_rastro = []
lista_leitura_rastro = []
lista_ts_medidor_rastro = []
...
def pegaBancoRastro(id_valor,ts_inicial,ts_final):
    con = psycopg2.connect(host='192.168.122.232', user='postgres', password='postgres',dbname='rastro')
    bd = con.cursor(cursor_factory = psycopg2.extras.DictCursor)
    ...
    for i in range(len(dados01)):
        ...
        sp = aux.split(",")
        lista_id_medidor_rastro.append(sp[0])
        lista_leitura_rastro.append(sp[1])
        lista_ts_medidor_rastro.append(sp[2])
        
    return (lista_id_medidor_rastro,lista_leitura_rastro,lista_ts_medidor_rastro)
